...
class Parse_seanse(db.Model):
    ...
    def get_all_for_user_id(user_id):
        return (
            db.session.query(Parse_seanse).filter(Parse_seanse.user_id == user_id).all()
        )
    ...
    def convert_all_img_from_url(self, page_to_parse):
        ...
        for url in urls:
            ...
            buff = FileLoader.save_from_url(
                os.path.join("parser_load", str(self.id)), url
            )
        ...
        for path in save_urls:
            ...
            FileConverter.convert_to(
                upload_folder,
                path,
                "WEBP",
                filename_save=_filename_save,
                parse_seanse_id=self.id,
            )
